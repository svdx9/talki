...
## What to build
- For each file in `backend/src/skills/catalog/*.yaml`, produce `backend-go/internal/skills/catalog/<id>.json`. Use any tool you like for the one-time conversion (`yq -o=json`, a small script). The JSON files are committed.
- `backend-go/internal/skills/skill.go`:
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Each `.yaml` file in `backend/src/skills/catalog/` has a matching `.json` file under `backend-go/internal/skills/catalog/`
